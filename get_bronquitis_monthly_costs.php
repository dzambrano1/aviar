...
try {
    ...
    $checkTableSql = "SELECT COUNT(*) as count FROM information_schema.tables 
                      WHERE table_schema = ? AND table_name = 'ah_bronquitis'";
    $checkStmt = $pdo->prepare($checkTableSql);
    $checkStmt->execute([$dbname]);
    $tableExists = $checkStmt->fetch(PDO::FETCH_ASSOC)['count'] > 0;
    
    if (!$tableExists) {
        echo json_encode(['error' => 'La tabla ah_bronquitis no existe en la base de datos.']);
        exit;
    }
    
    // Check if there's any data in the table
    $countSql = "SELECT COUNT(*) as count FROM ah_bronquitis";
    ...
    $checkFieldsSql = "SHOW COLUMNS FROM ah_bronquitis WHERE Field IN ('ah_bronquitis_fecha', 'ah_bronquitis_costo', 'ah_bronquitis_dosis')";
    $checkFieldsStmt = $pdo->prepare($checkFieldsSql);
    $checkFieldsStmt->execute();
    $fields = $checkFieldsStmt->fetchAll(PDO::FETCH_ASSOC);
    
    $requiredFields = ['ah_bronquitis_fecha', 'ah_bronquitis_costo', 'ah_bronquitis_dosis'];
    $foundFields = array_column($fields, 'Field');
    $missingFields = array_diff($requiredFields, $foundFields);
    
    if (!empty($missingFields)) {
        echo json_encode([
            'error' => 'Faltan campos requeridos en la tabla ah_bronquitis: ' . implode(', ', $missingFields)
        ]);
        exit;
    }
    
    // SQL query to get monthly bronquitis costs
    $sql = "SELECT 
                DATE_FORMAT(ah_bronquitis_fecha, '%Y-%m') AS month_key,
                DATE_FORMAT(ah_bronquitis_fecha, '%b %Y') AS month_label,
                SUM(ah_bronquitis_costo * ah_bronquitis_dosis) AS total_cost
            FROM 
                ah_bronquitis
            GROUP BY 
    ...
    if (empty($results)) {
        $checkNullsSql = "SELECT 
                            COUNT(*) as count,
                            SUM(CASE WHEN ah_bronquitis_fecha IS NULL THEN 1 ELSE 0 END) as null_dates,
                            SUM(CASE WHEN ah_bronquitis_costo IS NULL THEN 1 ELSE 0 END) as null_costs,
                            SUM(CASE WHEN ah_bronquitis_dosis IS NULL THEN 1 ELSE 0 END) as null_doses
                         FROM ah_bronquitis";
