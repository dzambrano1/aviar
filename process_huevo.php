...
if (isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'insert') {
            ...
            // Execute query
            $stmt = $conn->prepare("INSERT INTO ah_huevo (ah_huevo_tagid, ah_huevo_cantidad, ah_huevo_precio, ah_huevo_fecha) VALUES (?, ?, ?, ?)");
        ...
        // UPDATE operation
            ...
            // Execute query
            $stmt = $conn->prepare("UPDATE ah_huevo SET ah_huevo_tagid = ?, ah_huevo_cantidad = ?, ah_huevo_precio = ?, ah_huevo_fecha = ? WHERE id = ?");
        ...
        // DELETE operation
            ...
            // Execute query
            $stmt = $conn->prepare("DELETE FROM ah_huevo WHERE id = ?");
            $stmt->execute([$id]);
