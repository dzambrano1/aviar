...
try {
    ...
    // Query to get monthly aggregated death data from aviar table
    $query = "
        SELECT 
            DATE_FORMAT(deceso_fecha, '%Y-%m') AS month_year,
            COUNT(DISTINCT tagid) AS death_count,
            GROUP_CONCAT(DISTINCT deceso_causa SEPARATOR ', ') AS causes
        FROM 
            aviar
        WHERE 
            deceso_fecha IS NOT NULL 
            AND deceso_fecha != '0000-00-00'
        GROUP BY 
            DATE_FORMAT(deceso_fecha, '%Y-%m')
        ORDER BY 
            month_year ASC
    ";
    ...
    foreach ($monthlyResults as $monthData) {
        ...
        $detailQuery = "
            SELECT 
                deceso_causa AS cause,
                COUNT(DISTINCT tagid) AS count
            FROM 
                aviar
            WHERE 
                DATE_FORMAT(deceso_fecha, '%Y-%m') = :month_year
