...
try {
    ...
    $sql_counts = "
        SELECT
            a.etapa,
            COUNT(DISTINCT a.tagid) AS total_animals,
            COUNT(DISTINCT e.ah_encefalomielitis_tagid) AS vaccinated_animals
        FROM aviar a
        LEFT JOIN ah_encefalomielitis e ON a.tagid = e.ah_encefalomielitis_tagid
        WHERE a.etapa IN ('Inicio', 'Crecimiento', 'Finalizacion')
        GROUP BY a.etapa;
    ";
    ...
    $sql_list = "
        ...
        LEFT JOIN ah_encefalomielitis e ON a.tagid = e.ah_encefalomielitis_tagid
        WHERE a.etapa IN ('Inicio', 'Crecimiento', 'Finalizacion') 
            AND e.ah_encefalomielitis_tagid IS NULL
        ORDER BY a.etapa, a.nombre;
    ";
    ...
    error_log("Error fetching Encefalomielitis status by etapa: " . $e->getMessage());
