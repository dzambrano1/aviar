...
try {
    ...
    $checkTableSql = "SELECT COUNT(*) as count FROM information_schema.tables 
                      WHERE table_schema = ? AND table_name = 'ah_crd'";
    $checkStmt = $pdo->prepare($checkTableSql);
    $checkStmt->execute([$dbname]);
    $tableExists = $checkStmt->fetch(PDO::FETCH_ASSOC)['count'] > 0;
    
    if (!$tableExists) {
        echo json_encode(['error' => 'La tabla ah_crd no existe en la base de datos.']);
        exit;
    }
    
    // Check if there's any data in the table
    $countSql = "SELECT COUNT(*) as count FROM ah_crd";
    ...
    $checkFieldsSql = "SHOW COLUMNS FROM ah_crd WHERE Field IN ('ah_crd_fecha', 'ah_crd_costo', 'ah_crd_dosis')";
    $checkFieldsStmt = $pdo->prepare($checkFieldsSql);
    $checkFieldsStmt->execute();
    $fields = $checkFieldsStmt->fetchAll(PDO::FETCH_ASSOC);
    
    $requiredFields = ['ah_crd_fecha', 'ah_crd_costo', 'ah_crd_dosis'];
    $foundFields = array_column($fields, 'Field');
    $missingFields = array_diff($requiredFields, $foundFields);
    
    if (!empty($missingFields)) {
        echo json_encode([
            'error' => 'Faltan campos requeridos en la tabla ah_crd: ' . implode(', ', $missingFields)
        ]);
        exit;
    }
    
    // SQL query to get monthly crd costs
    $sql = "SELECT 
                DATE_FORMAT(ah_crd_fecha, '%Y-%m') AS month_key,
                DATE_FORMAT(ah_crd_fecha, '%b %Y') AS month_label,
                SUM(ah_crd_costo * ah_crd_dosis) AS total_cost
            FROM 
                ah_crd
            GROUP BY 
    ...
    if (empty($results)) {
        $checkNullsSql = "SELECT 
                            COUNT(*) as count,
                            SUM(CASE WHEN ah_crd_fecha IS NULL THEN 1 ELSE 0 END) as null_dates,
                            SUM(CASE WHEN ah_crd_costo IS NULL THEN 1 ELSE 0 END) as null_costs,
                            SUM(CASE WHEN ah_crd_dosis IS NULL THEN 1 ELSE 0 END) as null_doses
                         FROM ah_crd";
