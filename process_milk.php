...
if (isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'insert') {
            ...
            $requiredFields = ['tagid', 'milk', 'precio', 'fecha'];
            ...
            $stmt->execute([$tagid, $milk, $precio, $fecha]);
        ...
        // UPDATE operation
            ...
            // Validate required fields
            $requiredFields = ['id', 'tagid', 'milk', 'precio', 'fecha'];
            foreach ($requiredFields as $field) {
                if (!isset($_POST[$field]) || empty($_POST[$field])) {
                    throw new Exception("Campo requerido: $field");
                }
            ...
            // Execute query
            $stmt = $conn->prepare("UPDATE ah_leche SET ah_leche_tagid = ?, ah_leche_peso = ?, ah_leche_precio = ?, ah_leche_fecha = ? WHERE id = ?");
            $stmt->execute([$tagid, $milk, $precio, $fecha, $id]);
