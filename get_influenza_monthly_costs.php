...
try {
    ...
    $checkTableSql = "SELECT COUNT(*) as count FROM information_schema.tables 
                      WHERE table_schema = ? AND table_name = 'ah_influenza'";
    $checkStmt = $pdo->prepare($checkTableSql);
    $checkStmt->execute([$dbname]);
    $tableExists = $checkStmt->fetch(PDO::FETCH_ASSOC)['count'] > 0;
    
    if (!$tableExists) {
        echo json_encode(['error' => 'La tabla ah_influenza no existe en la base de datos.']);
        exit;
    }
    
    // Check if there's any data in the table
    $countSql = "SELECT COUNT(*) as count FROM ah_influenza";
    ...
    $checkFieldsSql = "SHOW COLUMNS FROM ah_influenza WHERE Field IN ('ah_influenza_fecha', 'ah_influenza_costo', 'ah_influenza_dosis')";
    $checkFieldsStmt = $pdo->prepare($checkFieldsSql);
    $checkFieldsStmt->execute();
    $fields = $checkFieldsStmt->fetchAll(PDO::FETCH_ASSOC);
    
    $requiredFields = ['ah_influenza_fecha', 'ah_influenza_costo', 'ah_influenza_dosis'];
    $foundFields = array_column($fields, 'Field');
    $missingFields = array_diff($requiredFields, $foundFields);
    
    if (!empty($missingFields)) {
        echo json_encode([
            'error' => 'Faltan campos requeridos en la tabla ah_influenza: ' . implode(', ', $missingFields)
        ]);
        exit;
    }
    
    // SQL query to get monthly influenza costs
    $sql = "SELECT 
                DATE_FORMAT(ah_influenza_fecha, '%Y-%m') AS month_key,
                DATE_FORMAT(ah_influenza_fecha, '%b %Y') AS month_label,
                SUM(ah_influenza_costo * ah_influenza_dosis) AS total_cost
            FROM 
                ah_influenza
            GROUP BY 
    ...
    if (empty($results)) {
        $checkNullsSql = "SELECT 
                            COUNT(*) as count,
                            SUM(CASE WHEN ah_influenza_fecha IS NULL THEN 1 ELSE 0 END) as null_dates,
                            SUM(CASE WHEN ah_influenza_costo IS NULL THEN 1 ELSE 0 END) as null_costs,
                            SUM(CASE WHEN ah_influenza_dosis IS NULL THEN 1 ELSE 0 END) as null_doses
                         FROM ah_influenza";
