...
if (isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'insert') {
            ...
            // Execute query
            $stmt = $conn->prepare("INSERT INTO ah_peso (ah_peso_tagid, ah_peso_animal, ah_peso_precio, ah_peso_fecha) VALUES (?, ?, ?, ?)");
        ...
        // UPDATE operation
            ...
            // Execute query
            $stmt = $conn->prepare("UPDATE ah_peso SET ah_peso_tagid = ?, ah_peso_animal = ?, ah_peso_precio = ?, ah_peso_fecha = ? WHERE id = ?");
        ...
        // DELETE operation
            ...
            // Execute query
            $stmt = $conn->prepare("DELETE FROM ah_peso WHERE id = ?");
            $stmt->execute([$id]);
