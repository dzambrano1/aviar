...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'insert' && isset($_POST['tagid'], $_POST['racion'], $_POST['producto'], $_POST['etapa'], $_POST['costo'], $_POST['fecha_inicio'], $_POST['fecha_fin'])) {
        try {
            ...
            // Insert into ah_sal table
            $stmt = $conn->prepare("INSERT INTO ah_sal (ah_sal_tagid, ah_sal_racion, ah_sal_producto, ah_sal_etapa, ah_sal_costo, ah_sal_fecha_inicio, ah_sal_fecha_fin) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $_POST['tagid'],
                $_POST['racion'],
                $_POST['producto'],
            ...
            $response = array(
                'success' => true,
                'message' => 'Registro agregado correctamente en ah_sal y aviar',
                'redirect' => 'aviar_registros.php'
            );
        ...
        try {
            $stmt = $conn->prepare("DELETE FROM ah_sal WHERE id = ?");
        ...
        try {
            ...
            // First, update the ah_sal table
            $stmt = $conn->prepare("UPDATE ah_sal SET ah_sal_racion = ?, ah_sal_producto = ?, ah_sal_etapa = ?, ah_sal_costo = ?, ah_sal_fecha_inicio = ?, ah_sal_fecha_fin = ? WHERE id = ?");
            $stmt->execute([
                $_POST['racion'],
                $_POST['producto'],
                $_POST['etapa'],
            ...
            $response = array(
                'success' => true,
                'message' => 'Registro actualizado correctamente en ah_sal y aviar',
                'redirect' => 'aviar_registros.php'
            );
